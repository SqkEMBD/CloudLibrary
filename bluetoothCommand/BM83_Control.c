...
#include "BM83_Control.h"

/***************************************************************************
 *
 * BM83-MCU I/O functions
 * • BM83-MFB 		(BT-WKUP)  pin mapped to PA4
 * • BM83-RST 		(BT-RST)   pin mapped to PC13
 * • BM83-UART-Rx 	(MCU-Tx)   pin mapped to PA2
 * • BM83-UART-Tx 	(MCU-Rx)   pin mapped to PA3
 * • BM83-WKUP-MCU (BM83 P0_0) pin mapped to PA5
 *
****************************************************************************/
...
{
  HAL_GPIO_WritePin(BM83_WKUP_GPIO_Port, BM83_WKUP_Pin, GPIO_PIN_RESET);
}

// set MFB pin to high
void BTM_Pin_MFB_SetHigh()
{
  HAL_GPIO_WritePin(BM83_WKUP_GPIO_Port, BM83_WKUP_Pin, GPIO_PIN_SET);
}
// set reset pin to low
void BTM_Pin_RESET_SetLow()
{
  HAL_GPIO_WritePin(BM83_RST_GPIO_Port, BM83_RST_Pin, GPIO_PIN_RESET);
}

// set reset pin to high
void BTM_Pin_RESET_SetHigh()
{
  HAL_GPIO_WritePin(BM83_RST_GPIO_Port, BM83_RST_Pin, GPIO_PIN_SET);
}
...
void MCU_UART_sendData(uint8_t *Data, uint16_t dataSize)
{
    ...
    {
	      HAL_UART_Transmit(&huart2, &Data[i], 1, 50);
    }
}
